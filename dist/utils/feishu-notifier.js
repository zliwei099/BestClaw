...
export class FeishuNotifier {
     ...
     * 通知任务开始
     */
    async notifyTaskStart(taskName, taskDescription) {
        return this.notify({
     ...
     * 通知任务完成
     */
    async notifyTaskComplete(taskName, commitHash, duration) {
     ...
     * 通知任务失败
     */
    async notifyTaskFailed(taskName, error) {
        return this.notify({
     ...
     * 通知迭代规划
     */
    async notifyIterationPlanned(taskName, phase) {
        return this.notify({
...
// 导出便捷函数
export function createNotifier(projectRoot) {
